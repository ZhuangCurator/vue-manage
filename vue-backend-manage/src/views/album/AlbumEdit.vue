...
<template>
  <div class="AlbumEdit">
    <custom-modal :title="modalTips"
      ...
      <Form ref="albumEdit" :model="dataForm" :rules="dataRules" label-position="top" >
        <FormItem label="专辑标题:" prop="title">
          <Input v-model="dataForm.title" :clearable="true"  />
        </FormItem>
        <FormItem label="分类:" prop="categoryId">
          <album-category-tree-select ref="categoryTreeSelect" :parentId="dataForm.categoryId"  :removed="0"  v-on:getParentCategoryId="getCategoryId"></album-category-tree-select>
        </FormItem>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getAlbum () {
        this.album({
          ...
          if (res.status === 1) {
            ...
            console.log('albumEdit#dataForm: ', res.fields)
          } else {
            this.$Notice.error({
              title: '错误',
              desc: res.msg
      ...
      modalConfirm () {
        console.log('点击了确定')
        this.$refs.albumEdit.validate((valid) => {
          if (valid) {
            this.albumEdit({
              id: this.id,
      ...
      // 弹窗关闭
      modalRemove () {
        this.$refs.albumEdit.resetFields()
        this.$emit('editClose')
      }
    },
    props: {
